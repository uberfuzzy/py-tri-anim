...
def main():
  ...
  if "i_size" not in cfg or "t_sizes" not in cfg:
    print("config.json must contain 'i_size' and 't_sizes' keys", file=sys.stderr)
    return 2
  ...
  out_html = template.replace("I_SIZE", json.dumps(i_size)).replace("T_SIZES", json.dumps(t_sizes))
